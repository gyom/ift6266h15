...
expdir = "/home/gyomalin/ML/tmp/exp07/"
...
def load_history(L_t):
    ...
    res = []
    for t in L_t:
        L_lion_json_path = [os.path.join(expdir, "maractus_exp07_%0.2d_lion_%d_log.json" % (t, lion_k)) for lion_k in [0,1]]
...
def run():
    ...
    for attribute_name in ['valid_accuracy', 'valid_loss', 'train_loss']:
        ...
        #output_path = os.path.join(expdir, "plot_exp07_%0.3d_%0.3d_%s.png" % (L_t[0], L_t[-1], attribute_name))
        output_path = "/home/gyomalin/Dropbox/umontreal/ift6266h15/ift6266h15/plots/exp07_lion_training/plot_exp07_%0.3d_%0.3d_%s.png" % (L_t[0], L_t[-1], attribute_name)
