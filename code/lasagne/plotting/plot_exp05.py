...
expdir = "/home/gyomalin/ML/tmp/exp05/"
...
def load_history(L_t):
    ...
    res = []
    for t in L_t:
        L_lion_json_path = [os.path.join(expdir, "maractus_exp05_%0.2d_lion_%d_log.json" % (t, lion_k)) for lion_k in [0,1]]
...
def run():
    ...
    output_path = os.path.join(expdir, "plot_%0.3d_%0.3d_%s.png" % (L_t[0], L_t[-1], attribute_name))
    plot(L_lion_tuple, output_path, attribute_name, dpi=150)

    # Something strange with 9 because it's doing only 65 epochs.
    # Maybe it has to do with an error when I ran the beginning of the script twice.
    # Skip that one.
    L_t = range(10,32+1)
    L_lion_tuple = load_history(L_t)
    output_path = os.path.join(expdir, "plot_%0.3d_%0.3d_%s.png" % (L_t[0], L_t[-1], attribute_name))
